...
MAX_THREADS = 1000
# Number of bytes that each thread will download in a iteration
CHUNK_SIZE = 1024*1024
BLOCK_SIZE = 8192
# File name to to save download in if file name is not specified with --filename
DEFAULT_FILE_NAME = 'download'

# Lock for progress bar since the library does not support multithreaded applications
...
class Downloader:
        ...
        while(1):
            ...
            try:
                ...
                if not task_interrupted:
                    self.queue.put({'start_location': start_location, 'end_location': end_location,
                                   'url': self.url, 'filename': self.file_name})
                    time.sleep(0.5)
                    task_interrupted = 1
                continue
    ...
    def __init__(self, url, file_name="", num_threads=DEFAULT_NUM_THREADS):
        ...
        try:
            ...
            raise ValueError(
                'URL is invalid or not connected to internet. Download could not be started')
        ...
        if not self.file_name:
            ...
            if mimetype == (None, None):
                raise Exception(
                    'Failed to extract file extension, please specify file name using --filename flag')
        ...
        self.bar = Bar('Downloading', max=self.queue.qsize(),
                       suffix='%(percent)d%%')
    ...
    def finish(self):
        ...
        for i in range(min(self.queue.qsize(), self.num_threads)):
            ...
            t = threading.Thread(target=self.threadHandler, kwargs={
                                 'start_location': task['start_location'], 'end_location': task['end_location'], 'url': task['url'], 'filename': task['filename']})
            t.setDaemon(True)
            t.start()
...
if __name__ == '__main__':
    ...
    downloader = Downloader(
        url=args.URL, file_name=args.fileName, num_threads=args.N_threads)
    downloader.finish()
